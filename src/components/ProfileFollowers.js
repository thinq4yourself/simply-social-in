...
class ProfileFollowers extends Profile {
  componentWillMount() {
  ...
  renderTabs() {
    return (
      <Menu pointing inverted secondary borderless size='massive' style={{ margin: '0 auto 0 30%', border: 'none' }}>
        ...
        <Menu.Item name='fav'><Link to={`/@${this.props.profile.username}/favorites`} className='ui text white'>2,542 Followers</Link></Menu.Item>
        <Menu.Item name='fav' active><Link to='#' className='ui text white'>517 Following</Link></Menu.Item>
      </Menu>
    );
  }
